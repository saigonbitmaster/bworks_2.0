#given a wallet or script name, select an utxo to use in lock/unlock transaction
BASEDIR=$(dirname $0)

echo "function base dir" ${BASEDIR}
getInputTx() {
	BALANCE_FILE=/tmp/walletBalances.txt
	rm -f $BALANCE_FILE
	echo 'Wallet Name: ' $1
	SELECTED_WALLET_NAME=$1
	${BASEDIR}/balance.sh $SELECTED_WALLET_NAME > $BALANCE_FILE
	SELECTED_WALLET_ADDR=$(cat ${BASEDIR}/wallets/$SELECTED_WALLET_NAME.addr)
	cat $BALANCE_FILE
...
getInputTxAuto() {
	...
	SELECTED_WALLET_NAME=$1
	TX_ROW=$(${BASEDIR}/balance.sh $SELECTED_WALLET_NAME | sort -k3,3rn | head -n 1)
	SELECTED_WALLET_ADDR=$(cat ${BASEDIR}/wallets/$SELECTED_WALLET_NAME.addr)
  #SELECTED_UTXO 1a8d47ed0efbdfd6df236fe4c89adbe9580b422ed0a6baf57d159141cfda7dd1#0
	SELECTED_UTXO="$(echo $TX_ROW | awk '{ print $1 }')#$(echo $TX_ROW | awk '{ print $2 }')"
  #SELECTED_UTXO_LOVELACE 10000000000
...
getScriptUtxo() {
	...
	SELECTED_WALLET_ADDR=$(cat ${BASEDIR}/wallets/$SELECTED_PLUTUS_SCRIPT.addr)
	TX_ROW=$(${BASEDIR}/balance.sh $SELECTED_PLUTUS_SCRIPT | grep ${2})
  #TX_ROW 1a8d47ed0efbdfd6df236fe4c89adbe9580b422ed0a6baf57d159141cfda7dd1     0        10000000000 lovelace + TxOutDatumNone
  #SELECTED_UTXO 1a8d47ed0efbdfd6df236fe4c89adbe9580b422ed0a6baf57d159141cfda7dd1#0
	SELECTED_UTXO="$(echo $TX_ROW | awk '{ print $1 }')#$(echo $TX_ROW | awk '{ print $2 }')"
  #SELECTED_UTXO_LOVELACE 10000000000
...
walletAddress() {
	WALLET_ADDRESS=$(cat ${BASEDIR}/wallets/$1.addr)
}
...
getScriptAddress() {
	SCRIPT_ADDRESS=$(cardano-cli address build --payment-script-file ${BASEDIR}/wallets/$1.plutus --testnet-magic $TESTNET_MAGIC_NUM)
  echo $SCRIPT_ADDRESS > ${BASEDIR}/wallets/$1.addr
}
