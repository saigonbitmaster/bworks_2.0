...
for (const [key, value] of Object.entries(filter)) {
  const includeOptions = key.includes('_gte') || key.includes('_gt') || key.includes('_lte') || key.includes('_lt') || key.includes('_ne')
  if (key.includes('_') && includeOptions)  {
    let [field, operator] = key.split('_') 
    newFilter.hasOwnProperty(field) ? newFilter[field] = {...newFilter[field], ['$' + operator]: value} : 
